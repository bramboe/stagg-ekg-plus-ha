...
class FellowStaggTargetTemperature(NumberEntity):
    ...
    _attr_native_unit_of_measurement = UnitOfTemperature.FAHRENHEIT  # Force Fahrenheit
    ...
    def __init__(self, coordinator: FellowStaggDataUpdateCoordinator) -> None:
        ...
        # Set min/max for Fahrenheit
        self._attr_native_min_value = 104  # 40°C
        self._attr_native_max_value = 212  # 100°C

        _LOGGER.debug(
            "Target temp range set to: %s°F - %s°F",
    ...
    def native_value(self) -> float | None:
        ...
        celsius = self.coordinator.data.get("target_temp")
        if celsius is not None:
            # Convert Celsius to Fahrenheit
            fahrenheit = (celsius * 9/5) + 32
            _LOGGER.debug("Target temperature read as: %s°F", fahrenheit)
            return fahrenheit
        _LOGGER.debug("Target temperature read as: None°F")
        return None

    async def async_set_native_value(self, value: float) -> None:
        """Set new target temperature."""
        _LOGGER.debug("Setting target temperature to %s°F", value)

        await self.coordinator.kettle.async_set_temperature(
            self.coordinator.ble_device,
            value,
            fahrenheit=True  # Explicitly tell it we're sending Fahrenheit
        )
